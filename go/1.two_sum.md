...
```go
    // Function that takes a slice of integers and a target, returns a slice of integers (the indices)
    func twoSum(nums []int, target int) []int {
        
        // Create an empty map
        seen := make(map[int]int)  // [int] = Key:   number from the array
                                   // int   = Value: index where that number was found
        
        // Loop through every element in nums
        for i, num := range nums {  // i   = current index
                                    // num = current value

            needed := target - num  // Calculate the number we need to reach the target

            if index, exists := seen[needed]; exists {
                
                // Check if we already seen the needed number
                // index = where that number was found
                // exists = true if the key exists in the map

                return []int{index, i}  // Return the two indices as a slice
            
            }

            seen[num] = i  // Store the current number and its index in the map

        }

        return nil  // This should never happen becaue the problem guarantees solution
